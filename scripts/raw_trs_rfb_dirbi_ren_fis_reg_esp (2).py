...
cols_pivot = [c for c in df.columns if '___' in c]  # todas as colunas pivotadas
...
df_unpivot = (
    df.select("CNPJ", "NOME", "UF", "CNAEPRINCIPAL", "PERIODOAPURACAO", "QTDBENEFICIOSCOMVALOR", "VALORCONSOLIDADO", "BEN_18_VAL_CONSOLID", F.expr(expr + " as (tipo_imposto, valor)") )
)
...
[f"'{c}', `{c}`" for c in cols_pivot]

# COMMAND ----------


expr = "stack({}, {})".format(len(cols_pivot),", ".join([f"'{c}', `{c}`" for c in cols_pivot]))
print(expr)

# COMMAND ----------

Importante entender o que e stack(2, 'A', `A`, 'B', `B`) F.expr(expr + " as (coluna, valor)")
